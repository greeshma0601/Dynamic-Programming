/*
Combination Sum - Part 2 Submissions: 9979   Accuracy: 31.16%   Difficulty: Medium   Marks: 4
Associated Course(s):   Interview Preparation
                 
Problems
Given an array of integers A and a sum B, find all unique combinations in A where the sum is equal to B.

ach number in A may only be used once in the combination.

Note:
   All numbers will be positive integers.
   Elements in a combination (a1, a2, … , ak) must be in non-descending order. (ie, a1 ≤ a2 ≤ … ≤ ak).
   The combinations themselves must be sorted in ascending order.
   If there is no combination possible the print "Empty" (without qoutes).
Example,
Given A = 10,1,2,7,6,1,5 and B(sum) 8,

A solution set is:

[1, 7]
[1, 2, 5]
[2, 6]
[1, 1, 6]

Input:
First is T , no of test cases. 1<=T<=500
Every test case has three lines.
First line is N, size of array. 1<=N<=12
Second line contains N space seperated integers(x). 1<=x<=9.
Third line is the sum B. 1<=B<=30.
 
Output:
One line per test case, every subset enclosed in () and in every set intergers should be space seperated.(See example)

Example:
Input:
2
7
10 1 2 7 6 1 5
8
5
8 1 8 6 8
12

Output:
(1 1 6)(1 2 5)(1 7)(2 6)
Empty


*/

#include<bits/stdc++.h>
#include<vector>
#include<iostream>
using namespace std;
void find_combi(vector<int> &arr,int target,vector<int> &temp,vector<vector<int>> &sol,int i,int total)
{
    if(i>arr.size()||total>target)
    return;
    if(total==target)
    {
        sol.push_back(temp);
        return;
    }
    temp.push_back(arr[i]);
    total=total+arr[i];
    find_combi(arr,target,temp,sol,i+1,total);
    temp.pop_back();
    total=total-arr[i];
    find_combi(arr,target,temp,sol,i+1,total);
}
void print(vector<vector<int>> sol)
{
    set<vector<int>> s;
    for(int i=0;i<sol.size();i++)
	s.insert(sol[i]);
    set<vector<int>>::iterator it=s.begin();
    while(it!=s.end())
    {
         vector<int> ans=*it;
         cout<<"(";
	     int j;
	     for(j=0;j<ans.size()-1;j++)
	     cout<<ans[j]<<" ";
	     cout<<ans[j]<<")";
	     it++;
    }
}
int main()
{
    int t,n,a,target;
    vector<int> arr;
    vector<int> temp;
    vector<vector<int>> sol;
    cin>>t;
    while(t--)
    {
        sol.clear();
        temp.clear();
        arr.clear();
        cin>>n;
        for(int i=0;i<n;i++)
        {
            cin>>a;
            arr.push_back(a);
        }
        cin>>target;
        sort(arr.begin(),arr.end());
        find_combi(arr,target,temp,sol,0,0);
        if(sol.size()==0)
        cout<<"Empty";
        else
        print(sol);
        cout<<"\n";
    }
}
